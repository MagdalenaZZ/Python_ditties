...
gatk --java-options "-Xmx29g" CollectAllelicCounts  -L %s \
 -I %s -R %s -O %s.allelicCounts.tsv' % (normpx,normpx,normpx,args.vcf,args.norm,args.gfa,normpx))

#Collect counts at the same sites for the case sample
print ('bsub -P Analysis -J CAC%s -o CAC%s.o -e CAC%s.e -n 1 -R "rusage[mem=30000]" -q bio \
gatk --java-options "-Xmx29g" CollectAllelicCounts -L %s \
-I %s -R %s -O %s.allelicCounts.tsv' % (tumpx,tumpx,tumpx,args.vcf,args.tum,args.gfa,tumpx) )
