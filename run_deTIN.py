...
print ('bsub -P Analysis -J CACn%s -o CACn%s.o -e CACn%s.e -n 1 -R "rusage[mem=16000]" -q bio \
gatk --java-options "-Xmx15g" CollectAllelicCounts  -L %s \
...
print ('bsub -P Analysis -J CACt%s -o CACt%s.o -e CACt%s.e -n 1 -R "rusage[mem=16000]" -q bio \
gatk --java-options "-Xmx15g" CollectAllelicCounts -L %s \
...
print ('bsub -P Analysis -J CRC%s -o CRC%s.o -e CRC%s.e  -n 1  -R "rusage[mem=5500]" -q bio  gatk --java-options "-Xmx5g" \
...
gatk PlotDenoisedCopyRatios --standardized-copy-ratios %s.standardizedCR.tsv --denoised-copy-ratios %s.denoisedCR.tsv \
...
--allelic-counts %s.tum.allelicCounts.tsv \
--normal-allelic-counts %s.norm.allelicCounts.tsv \
--output Segments \
...
# Remove the headers
#print ('cat %s.tum.allelicCounts.tsv | grep -v \'@\' > Segments/%s.norm.nohead.allelicCounts.tsv' % (normpx,normpx))
#print ('cat %s.tum.allelicCounts.tsv | grep -v \'@\' > Segments/%s.tum.nohead.allelicCounts.tsv' % (tumpx,tumpx))
print ('cat Segments/%s.hets.normal.tsv | grep -v \'@\' > Segments/%s.nohead.hets.normal.tsv' % (tumpx,tumpx))
print ('cat Segments/%s.hets.tsv | grep -v \'@\' > Segments/%s.nohead.hets.tsv' % (tumpx,tumpx))
print ('cat Segments/%s.modelFinal.seg | grep -v \'@\' > Segments/%s.modelFinal.nohead.seg' % (tumpx,tumpx))
...
print ('bsub -P Analysis -J DT%s -o deTIN/DT%s.o -e deTIN/DT%s.e  -n 1 -q bio python /home/mzarowiecki/bin/deTiN/deTiN/deTiN.py  \
--mutation_data_path %s.mutect1.stats  \
--cn_data_path Segments/%s.modelFinal.nohead.seg \
--tumor_het_data Segments/%s.nohead.hets.tsv \
--normal_het_data Segments/%s.nohead.hets.normal.tsv \
--exac_data_path /home/mzarowiecki/scratch/REF/exac.pickle \
--output_name %s \
--TiN_prior 0.01 \
--output_dir deTIN' % (tumpx,tumpx,tumpx,args.vcf, tumpx,tumpx,tumpx,tumpx) )
