...
fastn.file_to_dict(all_contigs_infile, contig_seqs)
...
for i in range(len(scaffolds)):
    ...
    seq = fastn.Fasta(scaff_name, ('').join([contig_seqs[c].seq for c in contig_list]))
...
fastn.deinterleave(reads_fastq, outprefix + '.reads_1.fq', outprefix + '.reads_2.fq')
...
for l in ref_seqs:
    seq = fastn.Fasta(''.join(l), ('N'*2*trim_bases).join([trimmed_contig_seqs[c].seq for c in l]))
    print(seq, file=f)
...
utils.syscall('scaffold_test_make_unique_tags.py ' + contigs_outfile + ' ' + ref_fasta + ' ' + contigs_outfile + '.tag')
