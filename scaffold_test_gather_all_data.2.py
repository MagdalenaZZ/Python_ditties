...
potential_joins = {
    'Staph.perfect.3kb.short': 940,
    'Staph.perfect.3kb.long': 940,
    'Staph.perfect.10kb.short': 27,
    'Staph.perfect.10kb.long': 27,
    'Staph.velvet': 167,
    'Rhodobacter': 570,
    '3D7.short': 9302,
    '3D7.long': 9302,
...
if options.use_sspace_iter:
    scaffolder.append('SSPACE.iter')

r_colours = {'MIP.bowtie-v0': 'gray',
    'MIP.bowtie-v3': 'gray',
    'MIP.bowtie2': 'gray',
    'MIP.bwa': 'gray',
    'OPERA.bowtie': 'red',
...
class ScaffResults:
    ...
    def __init__(self, bsub_o, log_file, max_joins, extra_cpu=0, extra_mem=0):
        ...
        if self.total_joins > 0:
            self.percent_joins_correct = self.stats['Correct joins'] / self.total_joins
        else:
            self.percent_joins_correct = 0

        self.percent_correct_joins_made = self.stats['Correct joins'] / self.potential_joins
...
#bad_runs.add(('human.combi', 'SOPRA.bowtie-v3'))
...
for dataset in datasets:
    ...
    for scaff in scaffolders:
        ...
        if (dataset, scaff) in extra_cpu:
            d[scaff] = ScaffResults(bsub_outfile, log_file, potential_joins[dataset], extra_cpu=extra_cpu[(dataset, scaff)], extra_mem=extra_mem[(dataset, scaff)])
        else:
            d[scaff] = ScaffResults(bsub_outfile, log_file, potential_joins[dataset])
